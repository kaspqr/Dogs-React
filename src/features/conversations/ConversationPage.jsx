...
} from "../messages/messagesApiSlice";
...
const ConversationPage = () => {
  ...
  const filteredIds = messages?.ids?.filter(
    (messageId) =>
  ...
  if (isSuccess) {
    ...
    const tableContent = filteredMessagesEntities?.length
      ? filteredMessagesEntities
          .slice(-displayedMessagesCount)
          .map((message) => <Message key={message.id} messageId={message.id} />)
      : null;

    return (
      <>
        <p className="conversation-page-username-title">
          <Link className="grey-link" to={`/users/${otherUser.id}`}>
            {otherUser.username}
          </Link>
        </p>
        <br />
        <div
          ...
          className="conversation-page-messages-div"
        >
          {tableContent}
        </div>
