...
const ConversationsList = () => {
  ...
  if (isLoading || isMsgLoading) alerts.loadingAlert("Looking for conversations")
  ...
  if (isSuccess && isMsgSuccess) {
    ...
    const filteredIds = ids.filter(conversationId => (
      entities[conversationId].sender === userId || entities[conversationId].receiver === userId
    ))

    const lastMessages = filteredIds.map(convoId => {
      const allMessages = msgIds.map(msgId => msgEntities[msgId])
      const conversationMessages = allMessages.filter(message => message.conversation === convoId)
